...
interest_map = {'Froid': 0, 'Tiède': 1, 'Chaud': 2}
...
tfidf = TfidfVectorizer(max_features=100, ngram_range=(1, 2), min_df=2, max_df=0.95, sublinear_tf=True)
...
positive_keywords = ['budget', 'urgent', 'signé', 'motivé', 'commande', 'qualité', 'rapide', 'chantier', 'disponible']
negative_keywords = ['trop cher', 'concurrent', 'hésit', 'pas de', 'difficile', 'rupture', 'limité']
...
# Combine features
numeric_features = ['Demande_Devis_enc', 'Connait_Marque_enc', 'Interest_Level_enc',
                    'notes_length', 'notes_word_count', 'positive_kw_count', 'negative_kw_count']
...
feature_names = numeric_features + keyword_features + list(decision_dummies.columns) + tfidf_feature_names
...
models = {
    'XGBoost': XGBClassifier(
        ...
        min_child_weight=3, scale_pos_weight=1, random_state=42,
        use_label_encoder=False, eval_metric='logloss', verbosity=0
    ),
    'LightGBM': LGBMClassifier(
        n_estimators=300, max_depth=6, learning_rate=0.05,
        subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=1.0,
        min_child_samples=20, num_leaves=31, random_state=42, verbose=-1
    ),
...
for name in model_names:
    ...
    comparison_data.append({
        'Model': name,
        'CV Acc': f"{cv['cv_accuracy_mean']:.4f}+/-{cv['cv_accuracy_std']:.4f}",
        'CV F1': f"{cv['cv_f1_mean']:.4f}+/-{cv['cv_f1_std']:.4f}",
        'CV AUC': f"{cv['cv_auc_mean']:.4f}+/-{cv['cv_auc_std']:.4f}",
...
for name in model_names:
    ...
    ax.plot(rec_vals, prec_vals, color=colors[name], lw=2.5,
            label=f'{name} (AP = {results[name]["avg_precision"]:.4f})')
ax.set_xlabel('Recall', fontsize=13)
ax.set_ylabel('Precision', fontsize=13)
ax.set_title('Precision-Recall Curves - Lead Conversion Classification', fontsize=15, fontweight='bold')
ax.legend(loc='best', fontsize=12)
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f'{output_dir}precision_recall_curves.png', dpi=200, bbox_inches='tight')
plt.close()
print("  - Precision-Recall curves saved")

# --- Metrics Comparison Bar Chart ---
fig, axes = plt.subplots(2, 3, figsize=(18, 10))
metrics_keys = ['accuracy', 'precision', 'recall', 'f1_score', 'auc_roc', 'avg_precision']
metrics_labels = ['Accuracy', 'Precision', 'Recall', 'F1 Score', 'AUC-ROC', 'Avg Precision']
x = np.arange(len(model_names))

for idx, (metric, label) in enumerate(zip(metrics_keys, metrics_labels)):
    row, col = idx // 3, idx % 3
    values = [results[name][metric] for name in model_names]
    bars = axes[row, col].bar(x, values, 0.5, color=[colors[n] for n in model_names], edgecolor='black', linewidth=0.5)
...
fig.suptitle('Model Performance Comparison - Lead Conversion', fontsize=16, fontweight='bold', y=1.01)
plt.tight_layout()
...
cv_metrics = ['cv_accuracy_mean', 'cv_f1_mean', 'cv_auc_mean']
cv_labels = ['CV Accuracy', 'CV F1', 'CV AUC-ROC']
...
for i, name in enumerate(model_names):
    ...
    bars = ax.bar(x + i * width, means, width, yerr=stds, label=name,
                  color=colors[name], edgecolor='black', linewidth=0.5, capsize=5)
...
plt.tight_layout()
plt.savefig(f'{output_dir}cv_stability.png', dpi=200, bbox_inches='tight')
plt.close()
...
ax.barh(range(n_top), top_importances[::-1], color=colors[best_model_name], edgecolor='black', linewidth=0.5)
...
plt.tight_layout()
plt.savefig(f'{output_dir}feature_importance_best_model.png', dpi=200, bbox_inches='tight')
plt.close()
...
for idx, name in enumerate(model_names):
    ...
    top_f = [feature_names[i] if i < len(feature_names) else f'feature_{i}' for i in top_idx]
    top_v = imps[top_idx]
    axes[idx].barh(range(15), top_v[::-1], color=colors[name], edgecolor='black', linewidth=0.5)
...
plt.tight_layout()
plt.savefig(f'{output_dir}feature_importance_all_models.png', dpi=200, bbox_inches='tight')
plt.close()
print("  - Feature importance (all) saved")

# --- Radar Chart ---
fig, ax = plt.subplots(figsize=(8, 8), subplot_kw=dict(polar=True))
categories = ['Accuracy', 'Precision', 'Recall', 'F1 Score', 'AUC-ROC', 'Avg Precision']
N = len(categories)
...
plt.tight_layout()
plt.savefig(f'{output_dir}radar_chart.png', dpi=200, bbox_inches='tight')
plt.close()
...
for name in model_names:
    report_data['cv_results'][name] = {k: round(float(v), 4) for k, v in cv_results[name].items()}
